...
namespace UVStudio
{
    class SQLiteHelper
    {
          ...
          params SQLiteParameter[] commandParameters)
        {
            ...
            if (commandParameters.Length > 0)
            {
                foreach (SQLiteParameter commandParameter in commandParameters)
                    sqLiteCommand.Parameters.Add(commandParameter);
            }
        ...
        public static SQLiteParameter CreateParameter(
          ...
          params SQLiteParameter[] commandParameters)
        {
            ...
            IDbCommand dbCommand = transaction == null || transaction.Connection != null ? (IDbCommand)transaction.Connection.CreateCommand() : throw new ArgumentException("The transaction was rolled back or committed, please provide an open transaction.", nameof(transaction));
            dbCommand.CommandText = commandText;
            foreach (SQLiteParameter commandParameter in commandParameters)
                dbCommand.Parameters.Add((object)commandParameter);
        ...
        private static SQLiteParameterCollection AttachParameters(
          SQLiteCommand cmd,
          string commandText,
          params object[] paramList)
        {
        ...
        protected internal static void AssignParameterValues(
          ...
          DataRow dataRow)
        {
            ...
            int num = 0;
            foreach (IDataParameter commandParameter in (IEnumerable)commandParameters)
            {
                if (commandParameter.ParameterName == null || commandParameter.ParameterName.Length <= 1)
                    throw new InvalidOperationException(string.Format("Please provide a valid parameter name on the parameter #{0},                            the ParameterName property has the following value: '{1}'.", (object)num, (object)commandParameter.ParameterName));
          ...
          params object[] parameterValues)
        {
            ...
            for (; index1 < length; ++index1)
            {
                if (commandParameters[index1].Direction != ParameterDirection.ReturnValue)
                {
                    ...
                    {
                        IDataParameter parameterValue = (IDataParameter)parameterValues[index2];
                        if (parameterValue.Direction == ParameterDirection.ReturnValue)
                            parameterValue = (IDataParameter)parameterValues[++index2];
                        commandParameters[index1].Value = parameterValue.Value != null ? parameterValue.Value : (object)DBNull.Value;
                    }
                    else
